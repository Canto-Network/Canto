...
func (suite *KeeperTestSuite) TestMsgCreateClawbackVestingAccount() {
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				testutil.FundAccount(s.app.BankKeeper, s.ctx, addr2, balances) //nolint:errcheck
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			testutil.FundAccount(s.app.BankKeeper, s.ctx, addr, balances) //nolint:errcheck
...
func (suite *KeeperTestSuite) TestMsgClawback() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			testutil.FundAccount(suite.app.BankKeeper, suite.ctx, addr, balances) //nolint:errcheck
